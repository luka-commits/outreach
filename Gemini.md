...
1. **Stability First**: When implementing new features, ALWAYS ensure they do not interfere with or break existing functionality. The codebase basis must remain stable. Test that existing features work as expected after any change.

2. **User Isolation**: Every Supabase query MUST filter by `user_id`
